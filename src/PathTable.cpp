...
bool PathTable::constrained(int from, int to, int to_time, int f_value)
{
    ...
    if (!goals.empty())
    {
        if (goals[to] <= to_time)
        {
            ...
            auto it = hit_agents.find(a);
            if (it == hit_agents.end() || it->second > f_value + goals[to] - to_time)
...
int PathTable::getHoldingTime(int goal_location, int earliest_timestep = MAX_TIMESTEP)
{
    if (table.empty())
        return -1;
    auto holding_time = (int) table[goal_location].size();
    while (holding_time > earliest_timestep && table[goal_location][holding_time - 1] == NO_AGENT)
        holding_time--;
