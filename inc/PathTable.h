...
public:
    ...
    vector<int> goals; // this stores the goal locations of the paths: key is the location, while value is the timestep when the agent reaches the goal
    std::map<int, int> hit_agents; // agents that have been hit by the constrained function, and the correspondiong f values

    void reset();
    void insertPath(int agent_id, const Path& path);
    void deletePath(int agent_id, const Path& path);
    bool constrained(int from, int to, int to_time, int f_value);
    int getHoldingTime(int goal_location, int earliest_timestep); // Return the earliest timestep when the agent can hold its goal location.
